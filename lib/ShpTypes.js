...
function forEachPointXY(buffer, offset, numPoints, fn) {
	var x,y;
	for( ;numPoints; numPoints--, offset += 8) {
...
module.exports = {
	...
	5: function polygon(buffer, jstsFactory) {
		...
		var parts = BReader.LE.arrayInt32(buffer, 44, numParts);

		forEachPointXY(buffer, 44 + 4 * numParts, numPoints, function(x, y) {
			//console.log('(%d, %d)', x, y);
		});
