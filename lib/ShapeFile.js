...
ShapeFile.prototype.intersects = function(geometry, options, callback) {
	...
	var envelope = geometry instanceof jsts.geom.Envelope ? geometry :
	geometry.getInternvalEnvelope();
	...
	var intersects = function(err, geometry, index) {
		if (err)
			callBackContext.error(err);
		else {
			if (geometry.instersects(geometry))
...
ShapeFile.prototype.shp = function() {
	if (!this._shp)
		this._shp = new Shp(this._baseName + '.shp', this.shx());
