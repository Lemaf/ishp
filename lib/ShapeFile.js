 ...
 * DBF based
 *
 * @param {Function} filter function(dbfRecord, index)
 * @param {Function} iterator function(feature, index)
 * @param {Function} done 
 * 
 */
ShapeFile.prototype.searchRecord = function(filter, iterator, done) {
	...
	/*
	 this.dbf().forEach(function(err, record, index, next) {
		if (!iterate)
			return;
	...
	*/
	...
	var shp = this.shp(), visit;

	function _iterator(record, index, next) {
		try {
			visit = filter(record, index);
		} catch (err) {
			next(err);
		...
		if (visit)
			shp.get(index, function(err, geometry) {
				if (err) {
					...
					var feature = {properties: record, geometry: geometry};

					try {
						iterator(feature, index, _next);
					} catch (e1) {
						_next(e1);
					}
				}
	...
	this.dbf().forEach(_iterator, _done);
};
