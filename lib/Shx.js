...
Shx.prototype._parseHeader = function(bytes, buffer, resolve, reject) {
	...
	this._offsetLimit = header.fileLength - 8;
...
Shx.prototype._get = function(index, callback) {
	...
	if (offset >= 100 && offset <= this._offsetLimit) {
		fs.read(this._fd, new Buffer(8), 0, 8, offset, function(err, bytes, buffer) {
			if (err)
				callback(err);
			else if (bytes !== 8)
				callback(new Error(format('Unexpected record bytes[%d]', bytes)));
			else {
				callback(null, {
					index: index,
					offset: buffer.readInt32BE(0),
					length: buffer.readInt32BE(4)
				});
			}
		});
	} else {
		callback(new Error(format('Invalid index[%d]', index)));
		return;
	}
