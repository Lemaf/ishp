...
NestedError = require('./NestedError');
...
function Shp(fileName, shx, jstsFactory) {
	...
	this._p = new Promise(function(resolve, reject) {
		shx._p.then(
			...
			function(err) {
				reject(NestedError('Shp - error in shx', err));
			}
		);
	});
}
...
Shp.prototype._loadBytes = function(shxRecord, callback) {
	...
	fs.read(this._fd, new Buffer(shxRecord.bytes), 0, shxRecord.bytes, shxRecord.offset * 2, function(err, bytes, buffer) {
		if (err)
			callback(err);
		else {
...
Shp.prototype._parseShape = function(shxRecord, bytes, buffer, callback) {
	...
	callback(null, geometry, shxRecord.index);
};
